# http_webhook_with_state

HTTP webhook example with generic JSON payload, request-header validation, and SurrealDB-backed state.
...
- How to use shared state to persist payloads in SurrealDB.
...
```bash
cargo run -p http_webhook_with_state
```
...
Before running, configure SurrealDB credentials and (optionally) HTTP auth:

```bash
export SURREALDB_ROOT_PASSWORD='CHANGE_ME'
